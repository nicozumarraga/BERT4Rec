...
class DataPreprocessing:
    def __init__(
        ...
        if os.path.exists(self.preprocessed_ratings_path):
            ...
            self.ratings = self.preprocess_ratings(
                self.load_ratings_as_df(self.ratings_path)
            )
    ...
    def _map_ratings_to_dense_sequence(self, ratings):
        ...
        """
        dense_movie_ids = {
            k: i + 2 for i, k in enumerate(sorted(list(ratings.movie_id.unique())))
        }
    ...
    def _reformat_into_list_and_drop_others(self, ratings):
        ...
        ratings = (
            ...
            .agg(list)
            .drop(["rating", "timestamp", "movie_id"], axis=1)
        )
        return ratings
